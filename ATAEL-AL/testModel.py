...
# 获取模型文件列表
model_server_baseline_files = utils.get_model_weight_files(args.data_name+"/"+args.model_name+'-'+str(args.level)+'-server')
print(model_server_baseline_files)
model_client_baseline_files = utils.get_model_weight_files(args.data_name+"/"+args.model_name+'-'+str(args.level)+'-client')
