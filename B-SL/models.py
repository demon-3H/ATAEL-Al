...
import torch
from BConv2d import Conv2d_Q

class Tnn_Bin_Conv2d(nn.Module):
    # 参数：last_relu-尾层卷积输入激活
    def __init__(self, input_channels, output_channels,
                 kernel_size=-1, stride=-1, padding=-1, groups=1,  A=2, W=2):
...
class MobileNetClient(nn.Module):
    def __init__(self,in_channels=3):
        ...
        self.conv1=Tnn_Bin_Conv2d(in_channels,32,kernel_size=3,stride=1,padding=1)
...
class ResNetClient(nn.Module):
    ...
    def __init__(self,in_channels=3):
        ...
        # 设置第一层的输入通道数
        self.inchannel = 64
        self.conv1 = Tnn_Bin_Conv2d(in_channels, 64, kernel_size=3, stride=1, padding=1)
...
class vgg16_conv_block(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.drop:
            x = self.dropout(x)
        return(x)
class vgg16_conv_block_bin(nn.Module):
    def __init__(self, input_channels, out_channels, rate=0.4, drop=True):
        super().__init__()

        self.conv = Tnn_Bin_Conv2d(input_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.dropout = nn.Dropout(rate)
        self.drop =drop
    def forward(self, x):
        x = self.conv(x)
...
def vgg16_layer_bin(input_channels, out_channels, num, dropout=[0.4, 0.4]):
    result = []
    result.append(vgg16_conv_block_bin(input_channels, out_channels, dropout[0]))
    for i in range(1, num-1):
        result.append(vgg16_conv_block_bin(out_channels, out_channels, dropout[1]))
    if num>1:
        result.append(vgg16_conv_block_bin(out_channels, out_channels, drop=False))
    result.append(nn.MaxPool2d(2,2))
    return(result)
class VGG16Client(nn.Module):
    def __init__(self,in_channels=3):
        super(VGG16Client,self).__init__()
        self.conv1=nn.Sequential(*vgg16_layer_bin(in_channels,64,2,[0.3,0.4]))
